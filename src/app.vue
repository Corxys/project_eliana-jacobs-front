...
const filters = computed( () => {
  ...
  const filtersByCategory = getters.filters(route.params.slug);

  if (!filtersByCategory) {
    return null;
  }

  const filterIsAlreadySelected = filtersByCategory
    .some((filter) => slugifyString(filter.name) === storedSelectedFilter);
 ...
 **/
const transitionScreen = computed(() => {
  ...
  >
    ...
    <main class="main">
      ...
      <div
        ...
        :class="{'main__container--stuck': isLoading || isMenuOpen || transitionScreen}"
      >
        <!-- Router -->
